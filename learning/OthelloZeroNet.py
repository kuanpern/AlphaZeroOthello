...
parser.add_argument('--inputNet', help="input Network (.dill)", type=str, required=False, default=None)
...
while True:
	...
	while True:
		...
		if _iter % 10 == 0:	# print every 500 mini-batches
			rootLogger.info('[iteration %d] loss: %.3f' % (_iter, running_loss / 500))
			running_loss = 0.0
		# end if
	# end for
	...
	if curtime - stime >= interval:
		...
		outfilename = outdir+'/'+outroot+'.'+str(curtime)+'.dill'
